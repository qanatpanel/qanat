...
ok(v.includes('path=' + encodeURIComponent('/abc123def/' + user.uuid)), 'VLESS path includes UUID');
ok(v.includes('fp=chrome'), 'VLESS fingerprint');
ok(v.includes('alpn=http/1.1'), 'VLESS alpn http/1.1');
ok(!v.includes('h2'), 'VLESS NO h2 alpn (WebSocket روی h2 با کلودفلر خراب است)');
...
ok(tr.includes('path=' + encodeURIComponent('/abc123def/' + 'a'.repeat(56))), 'Trojan path includes password');
